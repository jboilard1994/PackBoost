...
__global__ void _h_multiwarp_generic(
  ...
  for (int j = 0; j < stride; ++j) {
    ...
    for (int k = 0; k < 32; ++k) {
      ...
      // Per-k participation mask: either all-active or none (based on jj_k)
      const unsigned mask_k = __ballot_sync(__activemask(), jj_k < N);
      if (!mask_k) break;  // all lanes agree jj_k >= N → no more columns

      const int v = (int)((xbits >> k) & 1u);
      if (!v) continue;

      // Broadcast y & path from source lane k
      ...
      // d=1..max_depth-1
      #pragma unroll
      for (int d = 1; d < 10; ++d) {              // upper bound for unroll; runtime guard below
  ...
  if ((threadIdx.x >> 5) == 0) {
    ...
    for (int r = lane; r < rows_ge3 * 32; r += 32) {
      ...
      const int64_t fsum = (int64_t)sGE3[idx_sGE3(0, row, 0, lcol, rows_ge3)];
      const int64_t csum = (int64_t)sGE3[idx_sGE3(0, row, 1, lcol, rows_ge3)];
