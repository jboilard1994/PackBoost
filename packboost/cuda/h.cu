...
// --------- kernel ----------
template <typename LF_T>
__global__ void _h_multiwarp_fast(
    const uint32_t* __restrict__ XS,  // [nfeatsets, 32*M]
    const int32_t*  __restrict__ Y,   // [N]
    const LF_T*     __restrict__ LF,  // [nfeatsets, N] (u16/u32/u64)
    ...
    int nodes_total)
{
  ...
  for (int j = 0; j < stride; ++j) {
    ...
    if (jj_lane < N) {
      ...
      const LF_T lval = LF[(size_t)feat_set * (size_t)N + jj_lane];
      if constexpr (sizeof(LF_T) >= 8) {
        const uint64_t v = (uint64_t)lval;
        l_lo = (uint32_t)(v & 0xFFFFFFFFu);
        l_hi = (uint32_t)(v >> 32);
      } else {
        l_lo = (uint32_t)(uint64_t)lval;
        l_hi = 0u;
      }
    }
    // XS is exactly 32*M wide; base is multiple of 32 → no OOB
    const uint32_t xbits = XS[(size_t)feat_set * (size_t)cols_32M + (base + lane)];

    #pragma unroll
    for (int k = 0; k < 32; ++k) {
      const int jj_k = base + k;

      // per-k participation mask
      const unsigned mask_k = __ballot_sync(full_mask, jj_k < N);
      if (!mask_k) break;

      // do shuffles unconditionally, then gate by v
      const int32_t  yk   = __shfl_sync(mask_k, y_lane, k);
      const uint32_t loK  = __shfl_sync(mask_k, l_lo,   k);
      const uint32_t hiK  = __shfl_sync(mask_k, l_hi,   k);
      uint64_t lk = (uint64_t)loK | ((uint64_t)hiK << 32);
      ...
      // generic decode d=1..max_depth-1
      #pragma unroll
      for (int d = 1; d < 10; ++d) {   // unroll cap; guard below
        if (d >= max_depth) break;
        const uint64_t maskd = (1ull << d) - 1ull;
        const uint64_t tkd   = lk & maskd;
        lk >>= d;
  ...
  // --------- merge only d<=2 (tiny), O(log W) ----------
  for (int s = 0; (1 << s) < warps_per_block; ++s) {
    const int partner = (threadIdx.x >> 5) ^ (1 << s);
    __syncthreads();
    if ((((threadIdx.x >> 5) & ((1 << (s + 1)) - 1)) == 0) && partner < warps_per_block) {
      #pragma unroll
      for (int n = 0; n < 7; ++n) {
        const int self_f = idx_s02(threadIdx.x >> 5, n, 0, lane);
        const int self_c = idx_s02(threadIdx.x >> 5, n, 1, lane);
        const int part_f = idx_s02(partner,           n, 0, lane);
        const int part_c = idx_s02(partner,           n, 1, lane);
        s02[self_f] += s02[part_f];
        s02[self_c] += s02[part_c];
  ...
  // --------- drain: d<=2 by warp 0; d>=3 by ALL warps (per-warp drain) ----------
  if ((threadIdx.x >> 5) == 0) {
    // nodes 0..6
    #pragma unroll
    for (int n = 0; n < 7; ++n) {
  ...
  // each warp drains its own sGE3 slice (no inter-warp merge)
  {
    const int rows_ge3_local = rows_ge3;
    for (int r = lane; r < rows_ge3_local * 32; r += 32) {
      const int row  = r / 32;
      const int lcol = r % 32;
      const int node = 7 + row;
      const int64_t fsum = (int64_t)sGE3[idx_sGE3(wid, row, 0, lcol, rows_ge3_local)];
      const int64_t csum = (int64_t)sGE3[idx_sGE3(wid, row, 1, lcol, rows_ge3_local)];
      if (fsum) atomicAdd(Hptr(H, nodes_total, feat_set, node, 0, lcol), (unsigned long long)fsum);
      if (csum) atomicAdd(Hptr(H, nodes_total, feat_set, node, 1, lcol), (unsigned long long)csum);
    }
  }
...
static inline int heuristic_warps_per_block(int max_depth){
  ...
  if (lg2 < 0) lg2 = 0;
  return 1 << lg2; // up to 16
}



template <typename LF_T>
static void launch_h_multiwarp_fast_typed(
    ...
    cudaStream_t stream)
{
  ...
  int warps_per_block = heuristic_warps_per_block(max_depth);
  warps_per_block = min(warps_per_block, perf_cap_warps(max_depth)); // perf cap
  warps_per_block = max(1, min(warps_per_block, max_warps_by_smem)); // smem cap
  ...
  cudaFuncSetAttribute(_h_multiwarp_fast<LF_T>,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       (int)smem_bytes);

  _h_multiwarp_fast<LF_T><<<grid, block, smem_bytes, stream>>>(
    ...
    int max_depth)
{
  ...
  TORCH_CHECK(cudaGetLastError() == cudaSuccess,
              "h_multiwarp_fast launch failed: ", cudaGetErrorString(cudaGetLastError()));
  return H;
}
